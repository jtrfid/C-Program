...
%%%%%%%%%
\setbeamertemplate{section/subsection in toc}[circle]
...
%\include{lecture-4} % 数据类型, 运算符和表达式, 数学库函数，顺序程序设计举例

\part{选择结构程序设计}
\lecture{选择结构程序设计}{选择结构程序设计}
\frame{\partpage}
%\frame{\frametitle{Outlines}\tableofcontents[hideallsubsections]}
\frame{\frametitle{Outlines}\tableofcontents} 
\include{lecture-5} % ch4, 选择结构程序设计

\part{循环结构程序设计}
\lecture{循环结构程序设计}{pa3}
\frame{\partpage}
%\frame{\frametitle{Outlines}\tableofcontents[hideallsubsections]}
\frame{\frametitle{Outlines}\tableofcontents} 
...
%\include{lecture-8} % ch5, 循环结构程序设计举例(续)
...
%\include{lecture-14} % 第16次课, ch7, 嵌套与递归, 第17次课, 汉诺塔,全局变量与局部变量
%\include{lecture-15} % 第18次课, ch9, 讲到指针表示字符串常量
%\include{lecture-16} % 第19次课, 指针回顾, lecture-15剩余部分, 指针编程举例。
% lecture-16 期中考试总结, pow函数问题，移到第22次课讲
%\include{lecture-17} % 第20次课, 结构体
%\include{lecture-18} % 第21次课, 结构体编程举例续 
%%%%%%%%%%%%%%%%%%% last frame
\begin{frame}[plain]{}
  \begin{center}
    \begin{tikzpicture}
