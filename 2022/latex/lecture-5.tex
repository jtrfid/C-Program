...
\begin{frame}[shrink,fragile]{\small [例4.1 p84] 求$ax^2+bx+c=0$方程的根。$a,b,c$由键盘输入。}
\vspace{-0.2cm}
\begin{lstlisting}
#include<stdio.h>
...
\begin{frame}[shrink,fragile]{\small [例4.2 p85] 输入两个实数, 按由小到大的顺序输出这两个数。}
\vspace{-0.4cm}
\begin{columns}
...
\begin{frame}[shrink,fragile]{\small [例4.3 p86] 输入3个数a, b, c, 要求按由小到大的顺序输出。}
\vspace*{-0.4cm}
\begin{lstlisting}
#include<stdio.h>        
int main()                   
...
\begin{frame}[shrink,fragile]{if语句的一般形式}
\vspace{-0.2cm}
...
\begin{frame}[shrink,fragile]{特别注意: 数学表达式与C语言表达式的不同}
\vspace{-0.3cm}
\begin{lstlisting}
int a = 100;
...
\node[draw,text width=0.5\textwidth,rounded corners] (left) {	
	\begin{lstlisting}
	int a;
	scanf("%d",&a)
...
\node[draw,right,text width=0.5\textwidth,rounded corners] at (left.east) {
	\begin{lstlisting}
	// 等效于 
	int a;
	scanf("%d",&a)
...
\node[draw,text width=0.5\textwidth,font=\small,rounded corners] (left) {	
...
\begin{frame}[shrink,fragile]
给出一个百分制的成绩，要求输出成绩等级'A','B','C','D','E'。90分以上为'A'，$80\sim 89$分为'B',$70\sim 79$分为'C',$60\sim 69$分为'D'，60分以下为'E'。
\begin{lstlisting}
int grade;
scanf("%d",&grade);
