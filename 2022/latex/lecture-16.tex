...
\node[anchor=north west,text width=.8\textwidth] at($(a.south west)+(8mm,8mm)$) {
...
\node[text width=0.6\textwidth,anchor=north west] (b) at(a.north east) {
\vspace{-0.5cm}
\begin{lstlisting}
struct Student stu1,stu2;
stu1.num=10010;
...
\node[text width=0.6\textwidth,anchor=north,rounded corners,fill=green] at(b.south) {
``."成员运算符在所有运算符中优先级最高, 

\lstinline|stu2.num|可以当作一个整体看待。
};
...
\begin{frame}[shrink,fragile]{例1: 输入1个学生信息并输出统计数据}
\vspace{-0.5cm}
\begin{columns}[T]
\column{0.5\textwidth}
\begin{lstlisting}
...
\begin{frame}[shrink,fragile]{例2: 输入1个学生信息并输出统计数据(函数实现)}
\vspace{-0.5cm}
\begin{columns}[T]
...
void sort(struct Student stu[],int n)            
{
  ...
  for(i = 0; i < n-1; i++) 
  {
    ...
    for(j = i + 1; j < n; j++)
    {
       ...
       { 
         if(stu[j].score[0] > stu[k].score[0] || stu[j].score[1] > stu[k].score[1])
            k = j; 
       } 
    }
...
\node[anchor=north east,text width=0.5\textwidth,rounded corners,draw] at($(t.north east)+(-1cm,-0.5cm)$) {
	定义排序函数(选择法，降序): 
	
	按平均值排序，如果平均值相同，按照前2门课比较单科成绩. 
};
\end{tikzpicture}
\end{frame}
