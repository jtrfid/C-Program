...
\node[anchor=north,text width=0.4\textwidth,rounded corners,fill=green,align=center] (c) at($(b.south)+(0.5,0.7cm)$) {
	main函数: c=max(\&a,\&b);
};
...
\node[anchor=north west,text width=.5\textwidth] (b) at($(a.north east)+(-0.5,-1)$) {
	函数功能: 依据分级标准计算平均值及各级天数。
	
	函数参数说明: pm25[]: 输入PM2.5值, day[]: 输出各级对应的统计天数; n是pm25数组的长度, m是数组day的长度, 返回PM2.5平均值.
};
...
\begin{frame}[shrink,fragile]{例: 数字排序(联动输出解法一: 修改主程序)}
\vspace{-0.2cm}
给定n个整数，请计算每个整数各位数字和，按各位数字和从大到小的顺序输出。
...
\begin{frame}[shrink,fragile]{例: 数字排序(联动输出解法一: 修改排序子函数)}
\vspace{-0.3cm}
\begin{lstlisting}
void sort(int a[], int b[],int n) // a从大到小排序, b联动改变
{
...
\begin{frame}[shrink,fragile]{例: 数字排序(联动输出解法一: 修改输出子函数)}
\begin{lstlisting}
// 输出a,b
...
\begin{frame}[shrink,fragile]{例: 数字排序(联动输出解法二: 修改主程序)}
\vspace{-0.2cm}
给定n个整数，请计算每个整数各位数字和，按各位数字和从大到小的顺序输出。
...
\begin{frame}[shrink,fragile]{例: 数字排序(联动输出解法二: 修改排序子函数)}
\vspace{-0.3cm}
\begin{lstlisting}
void sort(int a[][2],int n) // a的第0列是整数, 第1列是其各位数字之和(排序列)
{
...
\begin{frame}[shrink,fragile]{例: 数字排序(联动输出解法二: 修改输出子函数)}
\begin{lstlisting}
