...
\begin{frame}[shrink,fragile]{while循环中, 用break,continue改变循环执行的状态}
\vspace{-0.2cm}
\centering
\begin{tikzpicture}
\node[] (aboveFig) {\includegraphics[scale=0.25]{breakContinue}};
\node[text width=0.45\textwidth,anchor=north east,rounded corners,draw,inner ysep=0pt] (leftFig) at(aboveFig.south) {
	\begin{lstlisting}
	while(表达式)
	{
		printf("语句1\n");
...
\node[text width=0.45\textwidth,anchor=north west,rounded corners,draw,inner ysep=0pt] at(aboveFig.south) {
	\begin{lstlisting}
	while(表达式) 
	{
		printf("语句1\n");
...
%\draw[dashed] (aboveFig.south) -- (aboveFig |- leftFig.south);
\end{tikzpicture}
\end{frame}

\begin{frame}[fragile]{while循环中, 体会break与continue的不同}
%\vspace{-0.5cm}
\centering
\begin{tikzpicture}%[font=\scriptsize]
%\node[text width=.5\textwidth,rounded corners,draw] (a) {
\node[text width=.5\textwidth,inner ysep=0pt] (a) {
	\begin{lstlisting}
	int i=0;
	while(i<4)
	{
...
\node[anchor=west,text width=.5\textwidth,inner ysep=0pt] at(a.east) (b) {
	\begin{lstlisting}
	int i=0;
	while(i<4)
	{
...
\begin{frame}{while循环中,break语句和continue语句的区别}
\includegraphics[scale=0.34]{forContinue}\\
\textcolor{blue}{注意: continue语句只结束本次循环，而非终止整个循环。break语句结束整个循环，不再判断执行循环的条件是否成立。}
\end{frame}

\begin{frame}[shrink,fragile]{ 注意: for循环的表达式3被视为下轮循环的首语句}
\vspace{-0.2cm}
\centering
\begin{tikzpicture}[node distance=0.4cm,font=\scriptsize]
\node[text width=0.75\textwidth,rounded corners,draw] (code) {
	...
	int main()
	{
		...
		{
			//终止本轮循环,但会执行for语句的表达式3(n++),开始下轮循环。表达式3被视为下轮循环的首语句。
			if (n%3==0) continue;
			printf("%d ",n);
		}
		printf("\n");
...
\node[anchor=west,initial left,draw] (b1) at($(code.north east)+(1.6cm,0)$) {表达式1};
\node[diamond,shape aspect=2,draw] (b2) [below= of b1] {表达式2};
\node[draw] (b3) [below= of b2] {continue之前语句};
\node[diamond,shape aspect=2,draw] (b4) [below=0.5cm of b3] {continue;};
\node[draw,blue] (b5) [below=0.5cm of b4] {continue之后语句};
\node[draw] (b6) [below= of b5] {表达式3};
\node[draw] (b7) [below= of b6] {for循环的下一条语句};
\node[fit=(b3) (b4) (b5),inner xsep=.5cm,xshift=0.2cm,draw=red,dashed]{};
\path[->]
(b1) edge[] (b2)
(b2) edge[] node[anchor=east]{Y} (b3)
(b3) edge[] (b4)
(b4) edge[] node[anchor=east]{N} (b5)
(b5) edge[] (b6)
;
\draw[->] (b4.east) -- +(0.5cm,0) node[anchor=south east]{Y} |- ($(b5)!.5!(b6)$);
\draw[->] (b6.south) -- ++(0,-0.2cm) -- +(-1.6cm,0) |- (b2.west);
\draw[->] (b2.east) -- node[anchor=south]{N} ++(1cm,0) |- (b7.east); 
\end{tikzpicture}
\end{frame}

\begin{frame}[shrink,fragile]{for循环中, break语句和continue语句的区别}
...
\begin{frame}[fragile]{for循环中, 体会break与continue的不同}
\vspace{-0.5cm}
\begin{columns}
...
\begin{frame}[shrink,fragile]{for循环中, 用break语句提前终止循环示例}
\vspace{-0.2cm}
