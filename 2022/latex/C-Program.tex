...
%\usepackage[thicklines,makeroom]{cancel} % 公式划掉，option [makeroom] to increase the horizontal spacing，[thicklines] asks for heavier lines and arrows
\usepackage[thicklines]{cancel} % 公式划掉
\renewcommand{\CancelColor}{\color{red}} % colored
...
%\includeonlylecture{part3} %循环结构程序设计
%\includeonlylecture{part4} %数组
%\includeonlylecture{part5} %函数
%%%%\includeonlylecture{part6} %指针，分解到函数的地址传递和最后一次的习题课中
%\includeonlylecture{part7} %结构体
%\includeonlylecture{part8} %习题课
...
\include{lecture-1} % 课程介绍，导论简介， C语言程序设计, 第1次课讲到ACSII编码表
\include{lecture-2} % 开发工具，算法，初识C语言程序(作业), 第2次课讲到算法特性之前
\include{lecture-3} % 初识C语言程序(讲解作业), 数据的输入输出, 第3次课平台演示讲解
\include{lecture-4} % 第4次课: 数据类型, 运算符和表达式,  数学库函数，顺序程序设计举例, 平台演示讲解。布置ppt 中的Exampe页为作业
...
\include{lecture-5} % ch4, 选择结构程序设计，以及讲解Example作业
...
%\frame{\frametitle{Outlines}\tableofcontents[hideallsubsections]}
\frame[shrink]{\frametitle{Outlines}\tableofcontents} 
\include{lecture-6} % ch5, 循环结构程序设计
\include{lecture-7} % ch5, 循环结构程序设计举例
\include{lecture-8} % ch5, 循环结构程序设计举例(续), 布置作业, 学习消化ppt中的所有代码

% 第9, 10课字符串处理函数即示例, 第11次课讲一半函数
\lecture{数组}{part4}
\part{数组}
\frame{\partpage}
\frame[shrink]{\frametitle{Outlines}\tableofcontents[hideallsubsections]} 
\include{lecture-9} % ch6, 一维数组，冒泡排序
\include{lecture-10} % ch6, 二维数组和字符数组
\include{lecture-11} % ch6, 字符串处理库函数及程序举例

% 第11次课讲一半为何使用函数，第12次讲实例, 第13次讲递归, 第14次半节课继续递归示例结束本章 
%(前面结合上机练习已经讲了函数和递归的用法--- 留到后面)
\lecture{函数}{part5}
\part{函数}
\frame{\partpage}
\frame[shrink]{\frametitle{Outlines}\tableofcontents[hideallsubsections]} 
\include{lecture-12} % ch7, 函数，定义，声明，调用，值传递，地址传递，模块化设计
\include{lecture-13} % ch7, 嵌套与递归, 汉诺塔,全局变量与局部变量

% 概念部分合并在函数的地址传递部分, 例题移至最后一次课的习题部分
%\lecture{指针}{part6}  
%\part{指针}
%\frame{\partpage}
%\frame[shrink]{\frametitle{Outlines}\tableofcontents[hideallsubsections]} 
%\include{lecture-14} % ch8, 指针 
%\include{lecture-15} % 示例

%%%%% 2021 第15课,第16次课开发平台讲示例：复试, 画图. 复习注意事项.
%% 2022 第14次课下半节, 讲结构体, 第15次课结构体示例, 第16次课习题课
\lecture{结构体}{part7}
\part{结构体}
\frame{\partpage}
\frame[shrink]{\frametitle{Outlines}\tableofcontents[hideallsubsections]} 
\include{lecture-16} %  结构体
\include{lecture-17} % 结构体编程举例续
