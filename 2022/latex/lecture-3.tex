...
\begin{frame}[fragile]{开发平台上演示讲解}
在开发平台，以具体的示例，详细讲解以下内容:
\begin{itemize}
	\item int, float, double, char 数据类型, sizeof(~)函数
	\item \%d, \%f, \%c, \%lf, \%x格式符的使用(见ppt中的表格)
	\item if(~)\{\quad\}, while(~)\{\quad\}简单语句
	\item \lstinline|char c; scanf("%c", &c);| 接收输入的字符
	
		\lstinline|char op; int x,y; scanf("%d%c%d",&x,&op,&y);|
		
	\item \lstinline|char c; c=getchar(); |接收输入的字符, \lstinline|putchar(c)|输出一个字符
