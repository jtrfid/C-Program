...
\section{二维数组的初始化: int a[3][3]=\{\{1,2,3\},\{4,5,6\},\{7,8,9\} \}}

\begin{frame}[shrink,fragile]{二维数组的初始化: int a[3][3]=\{\{1,2,3\},\{4,5,6\},\{7,8,9\} \}}
\vspace{-0.3cm}
\begin{lstlisting}
#define M 3 // 行数
#define N 3 // 列数
...
max=a[0][0]; // 首先认为a[0][0]最大, 这是max最好的初值
for(i=0;i<M;i++)  // 行
{
   for(j=0;j<N;j++) // 列
   {
...
\node[right,text width=.33\textwidth] (r) at($(4N.east)+(0.2,0)$) {
\vspace{-0.3cm}
\begin{lstlisting}
#define M 10 
int a[M][M],i,j;
...
\node[anchor=north west,text width=.38\textwidth] at($(r.north east)$) {
\vspace{-0.3cm}
\begin{lstlisting}
// 输出
for(i=0;i<M;i++) // 行
...
\draw[dashed,red] (r.north west) -- (r.south west);
\draw[dashed,red] (r.north east) -- (r.south east);
\end{tikzpicture}
\end{frame}
...
\node[draw,right,text width=.6\textwidth,rounded corners] (r) at($(4N.east)+(0.2,0)$) {
	\vspace{-0.3cm}
	\begin{lstlisting}
	#define M 10 
	int a[M][M],i,j;
